# Tareas Pendientes (Memory Backlog)

Tareas técnicas, refactorizaciones o ideas que surgen pero no son el foco inmediato.

### Workflow Audit
- [ ] (Future) Consider migrating `lib/data-provider.tsx` to Zustand store fully.

## Done
- [x] Audit Cycle: Extract `NavigationCard` component to reduce duplication.
- [x] Audit Cycle: Remove unused auth helpers (`canAccess`) in `lib/auth.tsx`.
- [x] Initial Project Setup
- [x] Snapshot Pre-Audit (Git)
- [x] Ejecutar Scripts de Auditoría
- [x] Verificar Hallazgos
- [x] Revisión Humana
- [x] Auto-Fix
- [x] Audit Cycle: Remove dead code (`hooks/useSyncStatus.ts`, `lib/error-handler.ts`).
- [x] Audit Cycle: Refactor `IndustrialIcons` to use base component.
- [x] Audit Cycle: Refactor `CalendarSummaryCard` to reduce UI duplication.
- [x] Snapshot Post-Fix (Git)

## Media Prioridad
- [ ] Revisar reporte de duplicados restantes.
- [ ] Poblar `project-knowledge` con más detalles de base de datos cuando se defina el esquema final.
- [ ] Documentar comandos personalizados si se crean workflows.

## Backlog
- [ ] Refinar convenciones de código a medida que el equipo crezca.
